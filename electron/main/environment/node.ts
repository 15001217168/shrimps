...
import SandboxEnvironmentService from './sandbox'

const LOG_SOURCE = 'NodeEnv'
...
export default class EnvironmentNodeService {
  /**
   * 步骤 1：生成对应系统的 Node.js 下载链接 (使用国内 npmmirror 镜像加速)
   */
  static getNodeDownloadInfo() {
   ...
   */
  static async downloadNodeBinary(url: string, dest: string) {
   ...
   * 步骤 3：解压文件到指定隐藏目录
   */
  static async extractNode(archivePath: string, platform: string) {
   ...
   */
  static getIsolatedEnv() {
    const platform = os.platform()
    // Windows 的 node.exe 在根目录，Mac/Linux 的 node 在 bin 目录下
    const binPath = platform === 'win32' ? NODE_DIR : path.join(NODE_DIR, 'bin')
   ...
   * 确保便携版 Node.js 环境就绪
   */
  static async ensureNodeEnvironment() {
   ...
   */
  static isPnpmInstalled(): boolean {
    try {
      const agentDir = SandboxEnvironmentService.getPaths().agent
   ...
   * 尝试修复环境问题
   */
  static async attemptFix(operationName: string, error: Error): Promise<void> {
   ...
   * 验证 Node 环境
   */
  static async validateNodeEnvironment(): Promise<boolean> {
    try {
      // 检查 Node 可执行文件
   ...
   * 修复 Node 环境（删除并重新安装）
   */
  static async repairNodeEnvironment(): Promise<boolean> {
   ...
   * 使用便携版环境执行命令
   */
  static async runWithPortableNode(
    command: string,
    args: string[] = [],
    options: any = {}
  ) {
    ...
    loggerInfo(`正在启动专属环境执行: ${command} ${args.join(' ')}`, LOG_SOURCE)
   ...
   * 测试沙盒环境
   */
  static async testNodeEnvironment() {
    try {
      const result = await this.runWithPortableNode('node', ['-v'])
      loggerSuccess(
   ...
   * 使用便携版安装包
   */
  static async installPackage(packageName: string, options: any = {}) {
