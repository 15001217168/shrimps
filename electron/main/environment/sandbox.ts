 ...
 * 沙箱环境管理器
 */
export default class SandboxEnvironmentService {
  /**
   * 获取沙箱目录路径
   */
  static getPaths() {
   ...
   */
  static isSandboxInitialized() {
    try {
      const platform = os.platform()
      const clawBin = platform === 'win32'
   ...
   * 构建独立沙箱环境
   */
  static async createIsolatedSandbox(openclawVersion = 'latest') {
    ...
    loggerInfo(`正在沙箱中安装 OpenClaw (${openclawVersion})...`, LOG_SOURCE)
   ...
   * 备份工作区
   */
  static async backupWorkspace(backupDir?: string) {
   ...
   * 恢复工作区
   */
  static async restoreWorkspace(backupPath: string) {
   ...
   * 打开工作区文件夹
   */
  static async openWorkspaceFolder() {
